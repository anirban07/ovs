                         ...
                         DB_FUNCTION_TABLE *pDbFnTable)
{
    ...
    SHASH_FOR_EACH (node, &svr->remotes) {
        ...
        ovsdb_jsonrpc_session_run_all(remote, pContext, pDbFnTable);
    }
}
...
static int ovsdb_jsonrpc_session_run(struct ovsdb_jsonrpc_session *,
    PDB_INTERFACE_CONTEXT_T pContext, DB_FUNCTION_TABLE *pDbFnTable);
    ...
    if (!jsonrpc_session_get_backlog(s->js)) {
        ...
        if (msg) {
            ...
            if (msg->type == JSONRPC_REQUEST) {
                ovsdb_jsonrpc_session_got_request(s, msg, pContext, pDbFnTable);
                              ...
                              DB_FUNCTION_TABLE *pDbFnTable)
{
    ...
    LIST_FOR_EACH_SAFE (s, next, node, &remote->sessions) {
        int error = ovsdb_jsonrpc_session_run(s, pContext, pDbFnTable);
        if (error) {
                                  ...
                                  DB_FUNCTION_TABLE *pDbFnTable)
{
    ...
    ret_error = pDbFnTable->pfn_db_add_session_to_context(pContext, s, request,
        monitor_cond_enable__, &reply);
    if (!strcmp(request->method, "transact") ||
        !strcmp(request->method, "convert")) {
        if (!reply) {
            ret_error = pDbFnTable->pfn_db_create_trigger(pDbFnTable, pContext,
                request);
            if (ret_error) {
                return;
            }
            // ovsdb_jsonrpc_trigger_create(pDbFnTable, pContext, request);
        }
        ...
        if (!reply) {
            ...
            enum ovsdb_monitor_version version = l ? OVSDB_MONITOR_V2
                                                   : OVSDB_MONITOR_V1;
            if (!ret_error) {
                reply = pDbFnTable->pfn_db_monitor_create(pContext,
                                                          request->params,
                                                          version, request->id);
            }
        }
        ...
        if (!ret_error) {
            reply = pDbFnTable->pfn_db_monitor_cond_change(pContext,
                                                           request->params,
                                                           request->id);
        }
        ...
        /* TODO change this to call the interface */
        if (!ret_error) {
            reply = pDbFnTable->pfn_db_monitor_cancel(pContext,
